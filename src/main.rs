...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        ...
        .init();
    let database_url =
        std::env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:data/checkins.db?mode=rwc".into());
    let uploads_dir =
    ...
    let state = Arc::new(AppState {
        ...
        uploads_dir,
        billing_base: std::env::var("BILLING_BASE_URL")
            .unwrap_or_else(|_| "https://pilot-api.sociobot.in".into()),
