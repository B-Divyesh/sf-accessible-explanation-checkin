...
function desktopOnly(testInfo: TestInfo) {
  test.skip(
    ...
    "One state-changing backend fixture is sufficient for this claim.",
  );
}
...
async function createCheckin(page: Page, title: string) {
  ...
  await page
    .getByLabel("Explanation prompt")
    .fill("Which choice changed your conclusion and why?");
...
test("@claim:demo-isolation starts with a populated review and keeps mutations in the demo namespace", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Watershed reasoning" }),
  ).toBeVisible();
  await page.getByLabel("Clear reasoning").first().check();
  await page
    .getByRole("button", { name: "Save sample review" })
    .first()
    .click();
  ...
  expect(
    await page.evaluate(() => ({
      demo: localStorage.getItem("demo:accessible-explanation-checkin:review"),
...
test("@claim:demo-reset restores the shipped sample", async ({ page }) => {
  ...
  await page
    .getByLabel("Private teacher note")
    .first()
    .fill("Changed only in demo");
  await page
    .getByRole("button", { name: "Save sample review" })
    .first()
    .click();
  await page.getByRole("button", { name: "Reset demo" }).click();
  await expect(page.getByLabel("Private teacher note").first()).toHaveValue(
    "Ask Maya to connect the model to the class data.",
  );
});

test("@claim:demo-exit-disposal discards edits before starting a real check-in", async ({
  page,
}) => {
  await page.goto("/?demo=1");
  await page
...
test("@claim:sample-csv-export downloads each shipped sample response", async ({
  page,
}) => {
  await page.goto("/demo");
...
test("@claim:keyboard-demo lets a teacher use the sample review with a keyboard", async ({
  page,
}) => {
  await page.goto("/demo");
  await page.keyboard.press("Tab");
...
test("@claim:no-account-needed creates a check-in without a sign-in step", async ({
  page,
}) => {
  await page.goto("/demo");
  await page.goto("/create");
...
test("@claim:recent-links-local keeps recent review links in one browser only", async ({
  browser,
  page,
}, testInfo) => {
  ...
  await page
    .getByLabel("Explanation prompt")
    .fill("Which example changed your conclusion?");
  await page.getByRole("button", { name: "Create private links" }).click();
  const reviewUrl = await page.locator("#review-link").inputValue();
  const saved = await page.evaluate(() =>
    JSON.parse(localStorage.getItem("recent-checkins") || "[]"),
  );
  expect(saved).toHaveLength(1);
  expect(saved[0]).toMatchObject({
    title: "Browser-only recent link",
    review: reviewUrl,
  });
  ...
  try {
    ...
    await expect(
      freshPage.getByText(
        "Your private review links will appear here after you create a check-in.",
      ),
    ).toBeVisible();
    expect(
      await freshPage.evaluate(() => localStorage.getItem("recent-checkins")),
    ).toBeNull();
  } finally {
...
test("@claim:voice-retention-control applies the selected free voice schedule", async ({
  ...
  await page.goto("/create");
  await expect(
    page.locator('select[name="voice_retention_days"] option'),
  ).toHaveText(["1 day", "3 days", "7 days"]);
  const response = await page.request.post("/api/checkins", {
    data: {
  ...
  await expect(
    page.getByText("it will delete automatically 3 days after submission", {
      exact: false,
    }),
  ).toBeVisible();
});

test("@claim:voice-recording-limits stops at 120 seconds and enforces the 4 MB boundary", async ({
  page,
}, testInfo) => {
  desktopOnly(testInfo);
  ...
  await page.goto("/demo");
  const create = await page.request.post("/api/checkins", {
    data: {
  ...
  await expect(
    page.getByText("Recording now… Select Stop recording when finished."),
  ).toBeVisible();
  expect(
    await page.evaluate(
      () =>
        (window as Window & { recordingTimerDelay?: number })
          .recordingTimerDelay,
    ),
  ).toBe(120_000);
  await page.evaluate(() =>
    (
      window as Window & { finishRecordingTimer?: () => void }
    ).finishRecordingTimer?.(),
  );
  ...
  const accepted = await page.request.post(
    `/api/checkins/${created.student_token}/submissions`,
    {
      data: {
...
test("@claim:free-response-limit enforces 35 responses under concurrent submissions", async ({
  page,
}, testInfo) => {
  desktopOnly(testInfo);
  await page.goto("/demo");
  ...
  const responses = await Promise.all(
    Array.from({ length: 40 }, (_, number) =>
      page.request.post(`/api/checkins/${student}/submissions`, {
        data: {
...
test("@claim:no-automated-judgment returns only student input and teacher-authored review fields", async ({
  page,
}, testInfo) => {
  desktopOnly(testInfo);
  await page.goto("/demo");
  ...
  await page.request.post(
    `/api/checkins/${created.student_token}/submissions`,
    {
      data: {
        student_name: "Alex",
        explanation_text: "I used the second example.",
        confidence: 2,
      },
    },
  );
...
test("@claim:offline-demo reloads after its first visit", async ({
  context,
  page,
}) => {
  await page.goto("/demo");
...
test("@claim:student-draft-local keeps offline edits in the browser and restores them after reconnecting", async ({
  context,
  page,
}) => {
  await page.goto("/demo");
  const { studentUrl } = await createCheckin(
    page,
    "Offline student draft proof",
  );
  ...
  await page
    .getByLabel("Write your explanation")
    .fill("I compared both examples.");
  ...
  await expect(
    page.getByText(
      "Offline. You can keep writing; submission needs a connection.",
    ),
  ).toBeVisible();
  await page
    .getByLabel("Write your explanation")
    .fill("I compared both examples and changed my conclusion while offline.");
  ...
  expect(
    ...
    student_name: "Riley Morgan",
    explanation_text:
      "I compared both examples and changed my conclusion while offline.",
    confidence: "5",
  });
  ...
  await expect(
    page.getByText(
      "Your writing is saved on this device; reconnect and send again.",
    ),
  ).toBeVisible();
  ...
  await expect(page.getByLabel("Write your explanation")).toHaveValue(
    "I compared both examples and changed my conclusion while offline.",
  );
...
test("@claim:student-keyboard-flow submits every required student field using only the keyboard", async ({
  page,
}, testInfo) => {
  desktopOnly(testInfo);
  await page.goto("/demo");
  ...
  await page.keyboard.press("Enter");
  await expect(
    page.getByRole("heading", { name: "Your check-in receipt" }),
  ).toBeVisible();
  await expect(
    page.getByText("I compared the examples and changed my answer."),
  ).toBeVisible();
});

test("@claim:student-review-workflow carries the explanation into a saved teacher review and free outputs", async ({
  page,
}, testInfo) => {
  desktopOnly(testInfo);
  await page.goto("/demo");
  const { studentUrl, reviewUrl } = await createCheckin(
    page,
  ...
  await page
    .getByLabel("Write your explanation")
    .fill("The comparison table changed my conclusion.");
  await page.getByLabel("Mostly").check();
  await page.getByRole("button", { name: "Send my explanation" }).click();
  await expect(
    page.getByRole("heading", { name: "Your check-in receipt" }),
  ...
  await expect(
    page.getByText("The comparison table changed my conclusion."),
  ).toBeVisible();
  await page.evaluate(() => {
  ...
  await page
    .getByLabel("Private teacher note")
    .fill("Ask Morgan to show the table.");
  ...
  await expect(page.getByLabel("Private teacher note")).toHaveValue(
    "Ask Morgan to show the table.",
  );
  await expect(page.getByLabel("Mark for follow-up")).toBeChecked();
  const download = page.waitForEvent("download");
  await page.getByRole("link", { name: "Export CSV" }).click();
  const csv = await download;
  const content = await csv.createReadStream().then(async (stream) => {
    const chunks: Buffer[] = [];
    for await (const chunk of stream!) chunks.push(Buffer.from(chunk));
    return Buffer.concat(chunks).toString("utf8");
  });
...
test("@claim:privacy-request-boundary keeps the demo and classroom flow on the product origin", async ({
  page,
}, testInfo) => {
  desktopOnly(testInfo);
  const requests: string[] = [];
  ...
  await page
    .getByLabel("Write your explanation")
    .fill("I checked the evidence twice.");
  ...
  expect(
    requests.some((url) =>
      /analytics|doubleclick|openai|google-analytics|segment/i.test(url),
    ),
  ).toBe(false);
});

test("@claim:billing-license-fixture handles an active and then revoked license verdict", async ({
  page,
}) => {
  let valid = true;
  await page.route(
    "https://api.sociobot.in/api/v1/products/accessible-explanation-checkin/verify**",
    (route) =>
      route.fulfill({
        status: 200,
  ...
  await expect(
    page.getByText("Classroom Plus is active on this device."),
  ).toBeVisible();
  valid = false;
  await page.getByRole("button", { name: "Verify license" }).click();
  await expect(
  ...
  await expect(
    page.getByRole("button", { name: "Create private links" }),
  ).toBeVisible();
});

test("@claim:refund-license-contract treats a recorded refunded license as revoked", async ({
  page,
}) => {
  await page.route(
    "https://api.sociobot.in/api/v1/products/accessible-explanation-checkin/verify**",
  ...
  await page.goto("/pricing");
  await expect(
    page.getByText("Sociobot/Dodo is the merchant of record."),
  ).toBeVisible();
  await expect(
    page.getByText(
      "Refunds are requested there; a refunded license becomes inactive here.",
    ),
  ).toBeVisible();
...
test("@claim:external-checkout verifies the live $39 checkout before leaving the product", async ({
  page,
  request,
}, testInfo) => {
  desktopOnly(testInfo);
  ...
  expect(liveCheckout.headers().location).toMatch(
    /^https:\/\/checkout\.dodopayments\.com\/session\/cks_[A-Za-z0-9]+$/,
  );
  ...
  await page.goto("/pricing");
  await expect(
    page.getByRole("heading", { name: "$39 one time" }),
  ).toBeVisible();
  const checkout = page.getByRole("link", {
    name: "Buy Classroom Plus through Sociobot (opens external site)",
  });
  await expect(checkout).toHaveAttribute("href", checkoutUrl);
  await checkout.click();
  await expect(page).toHaveURL(checkoutUrl);
  await expect(
    page.getByRole("heading", { name: "Sociobot checkout fixture" }),
  ).toBeVisible();
});
